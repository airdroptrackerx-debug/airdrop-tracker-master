...
export function AuthProvider({ children }: { children: ReactNode }) {
            ...
            if (userDoc.exists()) {
              ...
              if (streakResult.didIncrease || streakResult.didReset) {
                ...
                if (streakResult.achievedMilestone) {
                  console.log(
                    `🎉 Milestone achieved: ${streakResult.achievedMilestone} days!`
                  );
  ...
  const signUp = async (
    ...
    try {
      ...
      if (recaptchaToken) {
        console.log(
          "reCAPTCHA token received (would verify server-side in production)"
        );
      }
  ...
  const createOrUpdateSocialProfile = async (
    ...
    try {
      ...
      if (!userDoc.exists()) {
        ...
        const existingProviders =
          currentData.linkedProviders ||
          (currentData.provider ? [currentData.provider] : []);
        const linkedProviders = existingProviders.filter(
          (p: any) => p !== undefined && p !== null
        );
        ...
        const updateData = {
          ...
          emailVerified:
            firebaseUser.emailVerified !== undefined
              ...
              : currentData.emailVerified || false,
  ...
  const signInWithGoogle = async () => {
    try {
      ...
      if (error.code === "auth/popup-closed-by-user") {
        ...
        errorMessage =
          "Popup was blocked by your browser. Please enable popups for this site.";
  ...
  const signInWithTwitter = async () => {
    try {
      ...
      if (error.code === "auth/popup-closed-by-user") {
        ...
        errorMessage =
          "This domain is not authorized. Please add it to Firebase Console authorized domains.";
      } else if (error.code === "auth/operation-not-allowed") {
        errorMessage = "𝕏 sign-in is not enabled in Firebase Console.";
      } else if (error.code === "auth/invalid-credential") {
        errorMessage =
          "Invalid 𝕏 credentials. Please check your API Key and Secret in Firebase Console.";
      }
  ...
  const linkGoogleAccount = async () => {
    ...
    try {
      ...
      if (userDoc.exists()) {
        const currentData = userDoc.data();
        const linkedProviders = currentData.linkedProviders || [
