...
export function LoginForm() {
  ...
  // Check if user is in an embedded browser on mount
  useEffect(() => {
    setIsInEmbeddedBrowser(isEmbeddedBrowser());
  }, []);

  // Removed extension-related code
    ...
    try {
      if (isLogin) {
        ...
        if (currentUser) {
          ...
          // If verified, check onboarding status
        ...
        if (!recaptchaToken) {
          console.warn(
            "reCAPTCHA verification failed, but continuing with signup"
          );
        }
  ...
  const handleSocialAuth = async (provider: "google" | "apple" | "twitter") => {
    ...
    try {
      ...
      // Check if user needs onboarding (for social auth)
      const currentUser = auth.currentUser;
      if (currentUser) {
        // Social auth emails are usually pre-verified, but double-check
        if (!currentUser.emailVerified) {
          navigate("/verify-email");
          return;
        }
            ...
            >
              ...
              <div className="relative p-6 sm:p-8 space-y-6">
                <div ref={headerRef} className="text-center space-y-3">
                  ...
                  <h2 className="text-3xl font-bold dark:bg-gradient-to-r dark:from-primary dark:to-accent dark:bg-clip-text dark:text-transparent">
                    {isLogin ? "Welcome Back!" : "Join the Hunt"}
                  </h2>
                  <p className="text-muted-foreground">
                    {isLogin
                      ? "Sign in to continue your airdrop journey"
                      : "Start tracking airdrops today"}
                  </p>
                </div>

                {/* Show warning if in embedded browser */}
                {isInEmbeddedBrowser && <EmbeddedBrowserWarning />}
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  {!isLogin && (
                    <p className="text-xs text-center text-muted-foreground">
                      Protected by reCAPTCHA. By signing up, you agree to our
                ...
                <div ref={socialRef} className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <Button
                      ...
                      disabled={isSubmitting || isInEmbeddedBrowser}
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="text-sm">
                    ...
                    <p className="text-yellow-700">
                      ...
                      Password reset emails often end up there!
                    </p>
                  </div>
                </div>
              </div>
