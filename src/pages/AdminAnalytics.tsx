...
export default function AdminAnalytics() {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      // Note: Total tasks would require aggregation across all users
      // For now, we'll leave it as 0 or implement if needed
      const totalTasks = 0; // Implement if needed
      ...
      localStorage.setItem('cached_active_users_count', JSON.stringify({
  ...
  const statCards = [
    ...
    },
    {
      ...
      trend: ((analytics.activeUsers24h / analytics.totalUsers) * 100).toFixed(1) + '% of total',
    },
    {
      title: 'Active (7d)',
    ...
    },
    {
      ...
      value: analytics.totalAirdrops > 0 ? Math.round(analytics.airdropClicks / analytics.totalAirdrops) : 0,
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between mb-2">
                  <span className="text-sm text-muted-foreground">24h Active Rate</span>
                ...
                <div className="h-2 bg-muted rounded-full overflow-hidden">
                  <div 
                    ...
                    style={{ width: `${(analytics.activeUsers24h / analytics.totalUsers) * 100}%` }}
                  />
                </div>
              </div>
              <div>
                <div className="flex justify-between mb-2">
                  <span className="text-sm text-muted-foreground">7d Active Rate</span>
                  <span className="text-sm font-semibold">
                ...
                <div className="h-2 bg-muted rounded-full overflow-hidden">
                  <div 
                    ...
                    style={{ width: `${(analytics.activeUsers7d / analytics.totalUsers) * 100}%` }}
                  />
                </div>
              </div>
              <div>
                <div className="flex justify-between mb-2">
                  <span className="text-sm text-muted-foreground">Growth This Week</span>
                  <span className="text-sm font-semibold">
                ...
                <div className="h-2 bg-muted rounded-full overflow-hidden">
                  <div 
                    ...
                    style={{ width: `${Math.min((analytics.newUsersThisWeek / analytics.totalUsers) * 100, 100)}%` }}
                  />
                </div>
              </div>
            </div>
          ...
          <Card className="p-6 bg-gradient-to-br from-primary/10 to-accent/10 border-primary/20">
            ...
            <div className="space-y-4">
              <div className="p-4 bg-background/50 rounded-lg">
                <p className="text-sm font-semibold mb-2">💰 Referral Revenue Potential</p>
                <p className="text-2xl font-bold text-primary mb-1">
                  {analytics.airdropClicks} clicks
                </p>
                <p className="text-xs text-muted-foreground">
                  Your Explorer referral links have generated this many click-throughs
                </p>
              </div>
              
              <div className="p-4 bg-background/50 rounded-lg">
                <p className="text-sm font-semibold mb-2">📊 Ad Revenue Opportunities</p>
                <ul className="text-xs space-y-2 text-muted-foreground">
                  <li>• <strong>Google AdSense:</strong> ~$2-5 per 1000 page views</li>
                  <li>• <strong>Display Ads:</strong> Estimated ${((analytics.activeUsers24h * 5) / 1000 * 3).toFixed(2)}/day</li>
                  <li>• <strong>Affiliate Links:</strong> 5-20% commission per signup</li>
                </ul>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
            <div className="p-4 rounded-lg bg-muted/50">
              ...
              <p className="text-2xl font-bold">
                {((analytics.activeUsers7d / analytics.totalUsers) * 100).toFixed(0)}%
              </p>
            </div>
            <div className="p-4 rounded-lg bg-muted/50">
              <p className="text-muted-foreground mb-1">Daily Active Growth</p>
              <p className="text-2xl font-bold text-green-600">
                +{Math.round((analytics.newUsersToday / analytics.totalUsers) * 100)}%
              </p>
            </div>
            <div className="p-4 rounded-lg bg-muted/50">
              ...
              <p className="text-2xl font-bold">
                {analytics.totalAirdrops > 0 && analytics.activeUsers24h > 0
                  ? ((analytics.airdropClicks / (analytics.activeUsers24h * analytics.totalAirdrops)) * 100).toFixed(1)
                  : 0}%
              </p>
            </div>
            <div className="p-4 rounded-lg bg-muted/50">
